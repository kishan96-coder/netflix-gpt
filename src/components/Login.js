  ...
  return (
    <div>
      <div className='bg-gradient-to-b from-black to-white'>
        <div>
          <img src={BACKGROUND} className='fixed h-screen object-cover md:w-screen'/>
        </div>
        <Header/>
        <form onSubmit={(e)=>e.preventDefault()} className='bg-black  md:w-3/12 text-white z-10 absolute mt-40 md:ml-[38%] p-10 bg-opacity-80'>
        <h1 className='text-white p-6 font-bold  text-lg md:text-3xl'>{isSignIn?"Sign In":"Sign Up"}</h1>
            <input type="email" placeholder='Email Address' className='w-full bg-black bg-opacity-80 p-3 my-2' ref={email}/>
            {!isSignIn&&<input type="text" placeholder='name' className='w-full  bg-black bg-opacity-80 p-3 my-2' ref= {name} />}
            <input type="password" placeholder='password' className='w-full bg-black bg-opacity-80 p-3 my-2' ref={password}/>
